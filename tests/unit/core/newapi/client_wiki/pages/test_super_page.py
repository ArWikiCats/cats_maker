...
class TestMainPageInit:
    def test_sets_attributes(self, mock_login_bot):
        with patch("src.core.newapi.client_wiki.pages.super_page.change_codes", {"nb": "no"}):
...
class TestFalseEdit:
    ...
    @patch("src.core.newapi.client_wiki.pages.super_page.settings")
    def test_returns_false_when_no_fa_enabled(self, mock_settings, page):
        page.ns = 0
        mock_settings.bot.no_false_edit = True
        assert page.false_edit() is False

    @patch("src.core.newapi.client_wiki.pages.super_page.logger")
    @patch("src.core.newapi.client_wiki.pages.super_page.settings")
    ...
    def test_normal_edit_returns_false(self, mock_settings, page):
        page.ns = 0
        mock_settings.bot.no_false_edit = False
...
class TestCanEdit:
    ...
    @patch("src.core.newapi.client_wiki.pages.super_page.is_page_editable", return_value=True)
