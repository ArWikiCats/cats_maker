...
def check_last_edit_time(page, title_page, delay) -> bool:
    ...
    if timestamp:
        ...
        # logger.info(f"last-edit Δ={diff_minutes:.2f} min for {title_page}")

        wait_time = delay - diff_minutes

        if diff_minutes < delay:
            logger.info(f"Page:{title_page} last edit ({timestamp}).")
            logger.info(f"Page Last edit before {delay} minutes, Wait {wait_time:.2f} minutes. title:{title_page}")
            return False
